...
const formatLastGeneratedLabel = (lastGeneratedAt?: string | null) => {
  if (!lastGeneratedAt) return 'Not generated yet';
          ...
          <Animated.View entering={FadeInDown.duration(360)} style={styles.heroSection}>
            <Card style={styles.heroCard}>
              ...
              <View style={[styles.heroBadge, { borderColor: userBalance > 0 ? colors.success : userBalance < 0 ? colors.danger : colors.border, backgroundColor: userBalance > 0 ? `${colors.success}12` : userBalance < 0 ? `${colors.danger}12` : colors.cardElevated }]}>
                <Text style={[styles.heroBadgeText, { color: userBalance > 0 ? colors.success : userBalance < 0 ? colors.danger : colors.text_secondary }]}>{balanceStatus} • {formatCurrencyFromCents(Math.round(Math.abs(userBalance) * 100), groupCurrency)}</Text>
              </View>
            </Card>
          </Animated.View>
        ) : null}
